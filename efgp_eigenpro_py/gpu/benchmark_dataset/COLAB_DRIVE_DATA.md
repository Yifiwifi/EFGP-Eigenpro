...
## One master per compatible scale sweep

Compatible development sweeps may use exact row prefixes of one largest NPZ.
This rule must not be applied across different sampling or noise definitions.
Formal Stage-1 Synthetic instead matches the archived group_a/b/c notebook:
exact `_ntrainN` files generated with noise 0.3, seeds 20260421/1, 5M-row
chunks, and `N/4` test rows.
...
The controlled masters already in the workspace are:
...
Sizes above are decimal bytes. The 300M `_nN` Synthetic master uses noise 0.02
and belongs to the separate development-master route; it is not substituted
for formal `_ntrainN` Stage-1 data.
...
development master, not the formal Stage-1 family.
...
The formal notebook invokes the same generator automatically for missing
10M/30M/100M/300M sizes:
...
Register each exact input instead of assigning it master-prefix semantics. For
example, these are the distinct 10M synthetic-noise-0.3 and
spatial-stratified Winnebago inputs:
...
prefix API. These `_ntrainN` artifacts are the formal Synthetic route; the
noise-0.02 `_nN` master remains separate.
...
After the catalog contains Winnebago exact inputs, launch the campaign; missing
Synthetic exact files are generated locally. The prepared development/legacy upload is 13,847,768,160 bytes
(12.90 GiB). `checksums.sha256` must cover every NPZ/JSON artifact listed in the
...
colab_drive_upload_ready/
  ...
  data/
    ...
    synthetic-paper-noise03/  # optional legacy route
      synthetic_true_func_2d_ntrain*.{npz,json}
