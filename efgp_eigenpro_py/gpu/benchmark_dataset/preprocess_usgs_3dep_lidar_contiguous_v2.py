...
def _allocate_sample_counts(counts: np.ndarray, target: int, *, rng: np.random.Generator) -> np.ndarray:
    """Allocate an exact fixed-size random sample across files.

    If available, use the multivariate hypergeometric distribution: first decide
    how many points to take from each file, then sample uniformly within each
    file.  This is equivalent to a global uniform sample without materializing
    all points.  A deterministic proportional fallback is used for older NumPy.
    """
    ...
    if hasattr(rng, "multivariate_hypergeometric"):
        try:
            return np.asarray(rng.multivariate_hypergeometric(counts, target), dtype=np.int64)
        except Exception:
            pass

    # Fallback: proportional allocation with randomized remainder.
    ...
    if rem > 0:
        ...
        if eligible.size > 0:
            weights = frac[eligible]
            if not np.isfinite(weights).all() or weights.sum() <= 0:
                weights = np.ones_like(weights, dtype=np.float64)
            weights = weights / weights.sum()
            chosen = rng.choice(eligible, size=rem, replace=True, p=weights)
            for j in chosen:
                if alloc[j] < counts[j]:
                    alloc[j] += 1
                else:
                    # Rare saturation fallback.
                    free = np.flatnonzero(alloc < counts)
                    if free.size == 0:
                        break
                    alloc[int(rng.choice(free))] += 1
    return alloc
    ...
    *,
    ...
    for path in paths:
        ...
        if classification_keep is not None:
            ...
            n_kept = int(np.count_nonzero(np.isin(cls, np.asarray(classification_keep, dtype=cls.dtype))))
        else:
            n_kept = n_file
        n_after_file_cap = n_kept
        if max_points_per_file is not None and max_points_per_file > 0:
            n_after_file_cap = min(n_after_file_cap, int(max_points_per_file))
        count_rows.append(
            {
                "path": str(path),
                "tile_key": _tile_grid_key_from_name(path.name),
                "n_file": n_file,
                "n_kept_before_sample": int(n_kept),
                "n_after_file_cap": int(n_after_file_cap),
            }
        )
        effective_counts.append(int(n_after_file_cap))
        del las

    counts = np.asarray(effective_counts, dtype=np.int64)
    total_available = int(counts.sum())
    if total_available <= 0:
        raise ValueError("no points were available after classification filtering")
    ...
    for path, k, base_row in zip(paths, sample_counts.tolist(), count_rows):
        ...
        if classification_keep is not None:
            cls = np.asarray(las.classification)
            idx = np.flatnonzero(np.isin(cls, np.asarray(classification_keep, dtype=cls.dtype)))
        else:
            idx = np.arange(n_file, dtype=np.int64)
