...
def _source_manifest() -> dict[str, Any]:
    ...
    try:
        proc = subprocess.run(
            ["git", "status", "--short", "--untracked-files=all", "--", *relative_paths],
            cwd=str(repo_root),
            capture_output=True,
            text=True,
            check=True,
...
def _normalize_n_train(n_train: int | None) -> int | None:
    if n_train is None or int(n_train) == 0:
        return None
    if int(n_train) < 0:
        raise ValueError("n_train must be positive, zero, or None; zero means all rows.")
    return int(n_train)


def _resolve_dataset_dir(dataset_dir: str = "") -> Path:
    requested = str(dataset_dir).strip() or os.environ.get("BTAB_PROCESSED_DIR", "").strip()
    directory = Path(requested).expanduser().resolve() if requested else _PROCESSED_DIR.resolve()
    if not directory.is_dir():
        raise FileNotFoundError(f"processed dataset directory was not found: {directory}")
    return directory
...
def _load_dataset(
    ...
    if not path.exists():
        ...
        raise FileNotFoundError(f"processed dataset {stem!r} was not found; available: {available}")
    ...
    if mode == "prefix":
        try:
            ...
            y = load_stored_npz_prefix(path, "y_train", rows, dtype=np.float64).reshape(-1)
    ...
    return {
        ...
        "metadata_sha256": (
            hashlib.sha256(meta_path.read_bytes()).hexdigest() if meta_path.exists() else None
        ),
        "x": x,
        "y": y,
    }
...
def _validate_config(cfg: ControlledConfig) -> None:
    _normalize_n_train(cfg.n_train)
    if str(cfg.subset_mode).strip().lower() not in {"random", "prefix"}:
        raise ValueError("subset_mode must be 'random' or 'prefix'.")
    unknown = [method for method in cfg.methods if method not in _VALID_METHODS]
    if unknown:
        raise ValueError(f"unknown methods: {unknown}; choices are {sorted(_VALID_METHODS)}")
    ...
    if int(cfg.measured_repeats) < 5:
        raise ValueError("measured_repeats must be at least five for the controlled experiment.")
    ...
    if any(
        int(value) <= 0
        for value in (cfg.rank, cfg.nystrom_rank, cfg.rpcholesky_rank)
    ):
    ...
    if float(cfg.tol) < 100.0 * machine_eps:
        ...
        if not bool(cfg.allow_near_epsilon_tol):
            raise ValueError(message + " Use fp64 or explicitly set allow_near_epsilon_tol.")
        warnings.warn(message, RuntimeWarning, stacklevel=2)


def prepare_shared_system(cfg: ControlledConfig) -> PreparedSystem:
...
def resolve_score_box_rule(system: PreparedSystem, cfg: ControlledConfig) -> ResolvedBoxRule:
    ...
    if int(raw.box_idx.size) > budget:
        ...
        cumulative_radii = np.maximum.accumulate(np.abs(multi - center[None, :]), axis=0)
...
def resolve_method_specs(
    ...
    for method in cfg.methods:
        if method in {"cg", "jacobi"}:
            ...
            if int(rule.active.box_idx.size) == 0:
                raise ValueError("active-inverse requires a nonempty score-selected box.")
            ...
            if int(rule.active.box_idx.size) <= 1:
                raise ValueError("active-eig requires at least two modes in the score-selected box.")
            specs.append(
                MethodSpec(
                    label=method,
                    kind=method,
            ...
            specs.append(
                MethodSpec(
                    ...
                    selection_rule="global_gaussian_sketch",
                    result_role="baseline",
                )
            )
            ...
            specs.append(
                MethodSpec(
                    ...
                    selection_rule="randomized_residual_diagonal_pivots",
                    result_role="baseline",
                )
            )
    return specs, rule
...
def _fresh_operator_context(system: PreparedSystem, cfg: ControlledConfig) -> GPUOperatorContext:
    ctx = GPUOperatorContext()
    ctx.solve_dtype = _solve_dtype(system, cfg)
    return ctx


def _matvec_closure(system: PreparedSystem, op_ctx: GPUOperatorContext) -> Callable[[Any, Any], None]:
...
def _build_preconditioner(
    ...
    if kind == "jacobi":
        ...
        diag_inv = 1.0 / diag
        return make_jacobi_precond(backend, diag_inv), diag_inv, {
            "gamma": float(gamma),
    ...
    if kind == "nystrom":
        if spec.rank is None:
            raise RuntimeError("nystrom requires rank")
    ...
    if kind == "rpcholesky":
        if spec.rank is None:
            raise RuntimeError("rpcholesky requires rank")
        if system.data_ctx.xtxcol_gpu is None:
            system.data_ctx.xtxcol_gpu = xp.ascontiguousarray(
                backend.fft.ifftn(system.data_ctx.gf_gpu)
            )
...
def _is_converged(row: dict[str, Any], tol: float) -> bool:
    ...
    return status.startswith("converged") and math.isfinite(residual) and residual <= float(tol)
...
def summarize_rows(
    ...
    cg_by_repeat = {
        ...
        and math.isfinite(float(row.get("solve_seconds", math.nan)))
    }
    ...
    for method, method_rows in grouped.items():
        ...
        for row in method_rows:
            ...
            method_total = float(row.get("build_plus_solve_seconds", math.nan))
            if all(math.isfinite(v) and v > 0.0 for v in (cg_solve, method_total)):
        ...
        summary: dict[str, Any] = {
            ...
            "reuse_speedup_median": float(np.median(paired_reuse)) if paired_reuse else math.nan,
...
def pairwise_comparisons(
    ...
    for reference, candidate in pairs:
        ...
        repeat_ids = [
            ...
            and all(
                math.isfinite(float(by_method_repeat[(method, repeat)].get(key, math.nan)))
                and float(by_method_repeat[(method, repeat)].get(key, math.nan)) > 0.0
                for method in (reference, candidate)
                for key in ("build_plus_solve_seconds", "solve_seconds")
            )
        ]
        for repeat in repeat_ids:
            ref = by_method_repeat[(reference, repeat)]
            cand = by_method_repeat[(candidate, repeat)]
            ref_total = float(ref["build_plus_solve_seconds"])
            cand_total = float(cand["build_plus_solve_seconds"])
        ...
        if repeat_ids:
            ...
            if cand_build_median < ref_build_median and cand_solve_median > ref_solve_median:
                crossover = (ref_build_median - cand_build_median) / (
                    cand_solve_median - ref_solve_median
                )
                candidate_faster_through_rhs = float(max(int(math.ceil(crossover)) - 1, 0))
                crossover_status = "candidate_lower_build_higher_solve"
            elif cand_build_median <= ref_build_median and cand_solve_median <= ref_solve_median:
        ...
        comparisons.append(
            {
                ...
                "speedup_definition": (
                    "reference time / candidate time; values above one favor candidate"
                ),
            }
        )
    return comparisons
...
def run_controlled_experiment(cfg: ControlledConfig) -> Path:
    ...
    if cg_reference is None:
        raise RuntimeError("no converged measured CG solution is available as the fixed-system reference.")
    ...
    for label in {row.get("method") for row in rows if row.get("box_hash")}:
        ...
        if len(hashes) != 1:
            raise RuntimeError(f"active box changed across repeats for method {label!r}: {hashes}")
    ...
    manifest.update(
        {
            ...
            "default_selection_seconds": float(rule.selection_seconds),
            "timing_scope": (
                "Primary method time is preconditioner construction plus CG/PCG solve; "
                "the deployable default also includes its score-box selection cost. "
                "The one shared Fourier setup is reported separately."
            ),
    ...
    if diagnostic_arrays:
        np.savez_compressed(output_dir / "score_leverage_arrays.npz", **diagnostic_arrays)
    completion_payload = {
        "schema_version": 1,
        "system_id": system.system_id,
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--kernel", dest="kernel_family", default=ControlledConfig.kernel_family)
    parser.add_argument("--lengthscale", type=float, default=ControlledConfig.lengthscale)
    ...
    parser.add_argument("--lambda", dest="reg_lambda", type=float, default=ControlledConfig.reg_lambda)
    parser.add_argument("--fourier-eps", type=float, default=ControlledConfig.fourier_eps)
    ...
    parser.add_argument(
        ...
        help=(
            "Comma-separated: cg,jacobi,default,active-inverse,active-eig,"
            "full-eig,nystrom,rpcholesky."
        ),
    )
    ...
    parser.add_argument("--inverse-max-size", type=int, default=ControlledConfig.inverse_max_size)
    parser.add_argument("--rank", type=int, default=ControlledConfig.rank)
    parser.add_argument("--nystrom-rank", type=int, default=ControlledConfig.nystrom_rank)
    parser.add_argument(
        "--rpcholesky-rank", type=int, default=ControlledConfig.rpcholesky_rank
    )
    ...
    parser.add_argument("--warmup-repeats", type=int, default=ControlledConfig.warmup_repeats)
    parser.add_argument(
    ...
    parser.add_argument("--nystrom-seed", type=int, default=ControlledConfig.nystrom_seed)
    parser.add_argument(
        "--rpcholesky-seed", type=int, default=ControlledConfig.rpcholesky_seed
    )
    parser.add_argument(
        "--nufft-backend",
    ...
    parser.add_argument("--diagnostic-tol", type=float, default=ControlledConfig.diagnostic_tol)
    parser.add_argument(
        "--diagnostic-power-iter", type=int, default=ControlledConfig.diagnostic_power_iter
    )
    parser.add_argument(
        "--diagnostic-topk",
